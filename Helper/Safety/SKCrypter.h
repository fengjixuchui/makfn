...
namespace hack
{
    ...
    private:
        __inline constexpr void crypt(T* data)
        {
            for (int i = 0; i < _size; i++)
            {
                _storage[i] = data[i] ^ (_key1 + i % (1 + _key2) % (2 + _key3));
            }
        }
...
#define encrypt_key(str, key1, key2, key3) []() { \
			constexpr static auto crypted = hack::hack_crypter \
				<sizeof(str) / sizeof(str[0]), key1, key2, key3, hack::clean_type<decltype(str[0])>>((hack::clean_type<decltype(str[0])>*)str); \
					return crypted; }()

#define Encrypt(str) encrypt_key(str, __TIME__[4], __TIME__[7], __TIME__[6])